...
const caregiverLinks = caregiver.map((object) => {
    return (
        <div className=' border-solid border-black border-2 bg-white gap-0  grid grid-cols-2 grid-rows-2'>
            <img className='w-96 mt-16 ml-12 bg-white' src={object.img} />
...
const patient = [
    ...
    },
    {
        ...
        description:'The mission of Johns Hopkins Medicine is to improve the health of the community and the world by setting the standard of excellence in medical education, research and clinical care',
        img:JHM,
        linkText:'Patient Resources and Support Groups',
        id:'hopkins'
    },
    {
        ...
        description:'Oligo Nation was founded to bring the oligodendroglioma community together and mobilize around funding medical research for new, more effective treatments.',
        img:oligo,
        linkText:'Emotional Journey',
        id:'oligo'
    },
]
const patientLinks = patient.map((object) => {
    return (
        <div className='border-solid border-black border-2 bg-white gap-0 p-5 grid grid-cols-2' id={object.id}>
            
            <img className='w-96 ml-12 bg-white' src={object.img} alt={object.description} />
            <a className="underline text-blue-500 text-3xl mt-40" href= {object.link}> {object.linkText}</a>
